...
const Apply = () => {
  ...
  // TikTok: ViewContent when Apply page loads
  useEffect(() => {
    trackTikTokViewContent({
  ...
  // TikTok: InitiateCheckout when user reaches payment step (step 6)
  useEffect(() => {
    if (step === 6) {
      trackTikTokInitiateCheckout({
  ...
  const goToConfirmation = useCallback(() => {
    ...
    navigate(`/apply/${payload.supermarketSlug}/confirmation`, {
      replace: true,
      state: { confirmation: payload },
    });
  }, [buildConfirmationPayload, navigate]);

  // Safety net: if payment becomes completed while still on the payment step,
  // automatically go to the confirmation page (for Google Ads conversion URL).
  useEffect(() => {
  ...
  const nextStep = async () => {
    ...
    // TikTok: AddToCart when user selects a position (step 0 → 1)
    ...
    // TikTok: Identify + CompleteRegistration when personal info submitted (step 1 → 2)
    if (step === 1) {
      trackTikTokIdentify({
    ...
    if (step === 6) {
      // If payment is already confirmed, go to the confirmation page.
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        if (status === 'completed') {
          ...
          setIsPolling(false);
          // Navigate happens via the paymentStatus effect / goToConfirmation
          return;
        }
                ...
                {[
                  ...
                  <div key={field.label}>
                    ...
                    <input
                      ...
                      style={{
                        borderColor: field.label === "Email Address" && field.value.trim() && !emailValidation.valid
                          ? "#F59E0B"
                          : field.value ? brand.color : "#E5E7EB",
                      ...
                      <div className="mt-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                        ...
                        {emailValidation.suggestion && (
                          <button
                            ...
                            className="mt-1 ml-6 font-semibold underline underline-offset-2 hover:no-underline"
                          >
                    ...
                    {contactMethods.map((m, i) => (
                      ...
                      >
                        ...
                        <span className="text-xs" style={{ color: contactMethod === m.id ? "white" : "#9CA3AF" }}>
                          {m.id === "call" ? "Phone call" : m.id === "sms" ? "SMS" : m.id === "whatsapp" ? "WhatsApp" : "Email"}
                        </span>
                      </motion.button>
                    ))}
                  </div>
                  ...
                  >
                    <label className="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: brand.color + "15" }}>
                        {contactMethod === "email" ? (
                          <Mail className="h-3 w-3" style={{ color: brand.color }} />
                        ) : contactMethod === "whatsapp" ? (
                          <MessageCircle className="h-3 w-3" style={{ color: brand.color }} />
                        ) : (
                          <Phone className="h-3 w-3" style={{ color: brand.color }} />
                        )}
                      </div>
                    ...
                    <div className="relative">
                      <div className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: brand.color + "10" }}>
                        {contactMethod === "email" ? (
                          <Mail className="h-5 w-5" style={{ color: brand.color }} />
                        ) : contactMethod === "whatsapp" ? (
                          <MessageCircle className="h-5 w-5" style={{ color: brand.color }} />
                        ) : (
            ...
            >
              {step === 5
                ...
                ? paymentStatus === "completed"
                  ...
                  : paymentStatus === "processing"
                  ? "Processing..."
                  : `Pay KES ${brand.processingFee} via M-Pesa`
                : "Continue"}
              <ArrowRight className="h-4 w-4" />
            </button>
          </motion.div>
